...
Были успешно реализованы автотесты, проверяющие заполнение полей веб-формы и работу сохранения записей в базах данных MySQL и PostgreSQL.

## 2. Причины, по которым некоторые моменты не были реализованы:

Некоторые запланированные задачи не были реализованы по следующим причинам:

* Не реализована интеграция с системой CI (например, Appveyor CI) из-за сложности настройки и интеграции с текущей инфраструктурой проекта.
...
Запланированное время на автоматизацию составляло 2 недели. Фактически было затрачено 8 дней. 
